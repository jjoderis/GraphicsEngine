...
{
    // render camera info if the selected entity has one and keep
    // the camera alive while rendering
    if (std::shared_ptr<Engine::CameraComponent> camera =
            registry.getComponent<Engine::CameraComponent>(selectedEntity))
    {
        createComponentNodeOutline<Engine::CameraComponent>(
            "Camera",
            registry,
            camera.get(),
            [&]()
