...
{
    m_transformChangeCallback = registry.onUpdate<Engine::TransformComponent>(
...
const char *possible_rotation_systems[]{"Euler", "Angle Axis", "Quaternion"};

int possible_rotation_system_current = 0;

void UICreation::TransformComponentWindow::main()
{
    ImGui::DragFloat3("Translation", m_component->getTranslation().data(), 0.1);
    if (ImGui::IsItemEdited())
    {
        m_component->update();
        m_registry.updated<Engine::TransformComponent>(m_selectedEntity);
    }
    ImGui::DragFloat3("Scaling", m_component->getScaling().data(), 0.1);
    if (ImGui::IsItemEdited())
    {
        m_component->update();
        m_registry.updated<Engine::TransformComponent>(m_selectedEntity);
