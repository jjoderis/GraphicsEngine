...
public:
    ...
    void setClearColor(const Engine::Vector4 &color, int textureIndex = 0);
    void setClearColorI(const Engine::IVector4 &color, int textureIndex = 0);
    void clear();

    void getPixel(void *data,
                  int x,
                  int y,
                  unsigned int format,
                  unsigned int type,
...
private:
    ...
    std::map<int, Engine::Vector4> m_clearColors{};
    std::map<int, Engine::IVector4> m_clearColorsI{};

    std::vector<OtherAttachment> m_otherAttachments{
