    ...
    : m_cameraUBO{cameraUBO}, m_registry{registry}
{
    ...
    m_changeActive = m_registry.onAdded<ActiveCameraComponent>([this](unsigned int entity, std::weak_ptr<ActiveCameraComponent> active) {
    ...
    m_removeActive = m_registry.onRemove<ActiveCameraComponent>([this](unsigned int entity, std::weak_ptr<ActiveCameraComponent> active) {
...
void Engine::Systems::OpenGLCameraTracker::createInitActiveCB() {
    m_activeCB = m_registry.onAdded<CameraComponent>([this](unsigned int entity, std::weak_ptr<CameraComponent> camera) {
        if (m_registry.getComponents<Engine::CameraComponent>().size() == 1) {
            m_registry.addComponent<Engine::ActiveCameraComponent>(entity, std::make_shared<Engine::ActiveCameraComponent>());
        }
    });
}

void Engine::Systems::OpenGLCameraTracker::createUpdateActiveCB() {
    m_activeCB = m_registry.onUpdate<CameraComponent>(m_currentActiveCamera, [this](unsigned int entity, std::weak_ptr<CameraComponent> camera) {
        this->updateCameraBuffer(camera.lock());
    });
}

void Engine::Systems::OpenGLCameraTracker::makeActiveUnique() {
    std::vector<std::shared_ptr<ActiveCameraComponent>> activeCameras{m_registry.getComponents<ActiveCameraComponent>()};
